...
const WORKSPACE_DIR =
  '/Users/eastonplace/Library/Mobile Documents/com~apple~CloudDocs/Vibe Coding Projects';
const TOKEN_LOG_PATH = path.join(WORKSPACE_DIR, '.claude/agents/token-log.jsonl');
const USAGE_CACHE_PATH = path.join(WORKSPACE_DIR, '.claude/agents/usage-pct-cache.json');
...
function slugToDisplayPath(slug: string): string {
  ...
  // e.g. -Users-eastonplace-... -> /Users/eastonplace/...
...
function slugToDisplayName(slug: string): string {
  ...
  const SKIP_SEGMENTS = new Set([
    'Users', 'eastonplace', 'Library', 'Mobile', 'Documents',
    'com', 'apple', 'CloudDocs', 'Vibe', 'Coding', 'projects',
...
export async function getProjects(): Promise<ProjectInfo[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        sessionFiles = await listFiles(projectPath, '.jsonl');
      } catch {
        // no sessions
      }

      if (sessionFiles.length === 0) {
        // Still include project but with zeroed stats
        projects.push({
          slug,
      ...
      // Parse all session files for this project
      await Promise.all(sessionFiles.map((f) => parseSessionFile(f, acc)));

      // If lastActive wasn't found from timestamps, fall back to file mtime
      if (!acc.lastActive) {
        const stats = await Promise.all(sessionFiles.map((f) => fs.stat(f)));
        acc.lastActive = stats.reduce((max, s) => (s.mtimeMs > max ? s.mtimeMs : max), 0);
      }
...
export async function getTokenLog(): Promise<TokenLogEntry[]> {
  const raw = await readJSONL<TokenLogEntry>(TOKEN_LOG_PATH);
...
export async function getLiveUsage(): Promise<LiveUsage> {
  ...
  try {
    const content = await fs.readFile(USAGE_CACHE_PATH, 'utf-8');
...
export async function getToolUsage(): Promise<ToolUsageEntry[]> {
  ...
  // Track which tool_use IDs had errors (from subsequent tool_result blocks)
  const toolUseIdToName: Record<string, string> = {};

  function handleEntry(entry: any) {
    // Tool use is always in assistant message.content[]
    if (entry.type === 'assistant') {
      const msg = entry.message;
      if (!msg) return;
      ...
      // Attribute turn token usage evenly across all tool calls in this turn
      const usage = msg.usage ?? {};
      const turnTokens =
      ...
      const tokensPerTool = toolUseBlocks.length > 0
        ? Math.round(turnTokens / toolUseBlocks.length)
        : 0;

      for (const block of toolUseBlocks) {
        const toolName: string = block.name ?? 'unknown';
        if (!toolCounts[toolName]) toolCounts[toolName] = { count: 0, errorCount: 0, tokens: 0 };
        toolCounts[toolName].count += 1;
        toolCounts[toolName].tokens += tokensPerTool;
        // Map the tool_use id -> name for error tracking
        if (block.id) toolUseIdToName[block.id] = toolName;
      }
    }

    // Tool results are in user message.content[] (the follow-up turn)
    if (entry.type === 'user') {
      const content: any[] = entry.message?.content ?? [];
      for (const block of content) {
        if (block.type === 'tool_result' && block.is_error) {
          // Map back to tool name if we have it
          const toolName = (block.tool_use_id && toolUseIdToName[block.tool_use_id])
  ...
  try {
    ...
    for (const project of projectEntries) {
      ...
      const sessionFiles = await listFiles(projectPath, '.jsonl');
      for (const sessionFile of sessionFiles) {
        await parseJSONLStream(sessionFile, handleEntry);
      }
    }
  } catch {
